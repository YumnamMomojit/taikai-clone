...
# 🚀 Taikai.network Clone - Upload and Deploy to EC2
# Usage: ./upload-and-deploy.sh [EC2_PUBLIC_IP] [SSH_KEY_PATH]
...
ARCHIVE_NAME="taikai-deploy-package.tar.gz"

# Print banner
echo -e "${BLUE}"
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║              🚀 Taikai.network Clone                       ║"
echo "║                 Upload & Deploy to EC2                       ║"
echo "╚══════════════════════════════════════════════════════════════╝"
echo -e "${NC}"

# Check if EC2 IP is provided
if [ -z "$EC2_IP" ]; then
    echo -e "${YELLOW}Usage: ./upload-and-deploy.sh [EC2_PUBLIC_IP] [SSH_KEY_PATH]${NC}"
    echo -e "${YELLOW}Example: ./upload-and-deploy.sh 54.123.45.67 /path/to/your-key.pem${NC}"
    exit 1
fi
...
# Create a comprehensive deployment script that includes everything
print_status "Creating comprehensive deployment script..."

cat > "$TEMP_DIR/deploy-comprehensive.sh" << 'EOF'
#!/bin/bash

# 🚀 Taikai.network Clone - Comprehensive EC2 Deployment
set -e
...
print_status "Starting comprehensive deployment of $APP_NAME..."
...
echo ""
echo "🎯 Next Steps:"
echo "  1. Test the application at http://$(curl -s http://169.254.169.254/latest/meta-data/public-ipv4)"
...
# Make the comprehensive deployment script executable
chmod +x "$TEMP_DIR/deploy-comprehensive.sh"
...
print_status "Uploading deployment package to EC2 instance..."
$SCP_CMD "$TEMP_DIR/$ARCHIVE_NAME" "ubuntu@$EC2_IP:/tmp/"

# Extract and run the deployment
print_status "Extracting and running deployment on EC2 instance..."
$SSH_CMD "
    cd /tmp &&
    tar -xzf $ARCHIVE_NAME &&
    sudo bash deploy-comprehensive.sh
"

# Clean up
print_status "Cleaning up temporary files..."
rm -rf "$TEMP_DIR"
$SSH_CMD "rm -f /tmp/$ARCHIVE_NAME"

print_success "🎉 Upload and deployment completed successfully!"
echo ""
echo "📋 Access Information:"
